...
class Db {
  ...
  void dbTables(Database database, int version) async {
    ...
    await database.execute(
        'CREATE TABLE IF NOT EXISTS `users` (`id`	INTEGER PRIMARY KEY AUTOINCREMENT, `name`	varchar ( 100 ) DEFAULT NULL,`username`	varchar ( 100 ) DEFAULT NOT NULL,`password`	varchar ( 50 ) DEFAULT NULL,`phoneNumber`	varchar ( 20 ) DEFAULT NULL,`role`	varchar ( 20 ) DEFAULT `geust`);');
    //exercise table
    await database.execute(
        'CREATE TABLE IF NOT EXISTS `exircises` (`id` INTEGER PRIMARY KEY AUTOINCREMENT,`startDate`	TIMESTAMP ,`endDate`	TIMESTAMP,`Statut`	BOOLEAN NOT NULL DEFAULT 0);');
    // config general table
    await database.execute(
        'CREATE TABLE IF NOT EXISTS `generalConfig` (`id` INTEGER PRIMARY KEY AUTOINCREMENT,`name`	varchar ( 50 ) DEFAULT NULL,`value`	text);');
    // etablishement table
    await database.execute(
    ...
    await database.execute(
        'CREATE TABLE IF NOT EXISTS `unitOfMeasure` (`id`	INTEGER PRIMARY KEY AUTOINCREMENT,`name`	varchar ( 100 ) DEFAULT NULL,`symbole`	varchar (5) DEFAULT NULL);');
    // currencies table
    await database.execute(
  ...
  Future<dynamic> updateActive(String query,  List<Object?>? whereArgs) async {
    Database db = await this.db;
    var result =
        await db.rawUpdate(query, whereArgs);
    if (result > 0) {
      var data = await db.rawQuery("select * from exercises where id=$whereArgs");
      return {"status": 200, "data": data.toList()};
    } else {
      return {"status": 400};
    }
